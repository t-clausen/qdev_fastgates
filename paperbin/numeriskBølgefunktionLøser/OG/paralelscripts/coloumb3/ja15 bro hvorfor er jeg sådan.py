...
if case == "coloumb":
    ...
    if betterColoumb == True:
        for i,__ in enumerate(funcPots):
            funcPots[i] = 0
        siliconCords = np.full((int((coloumbAmm*2-1)**3),3),0,dtype=float)
        siliconCellWidth = cellRad*2
        dec = len(str(siliconCellWidth).split('.')[1])
        dec = abs(impDec)+2
        dcA = coloumbAmm*2-1
        dcARange = range(0,dcA)
        for i in dcARange:
            for j in dcARange:
                for k in dcARange:
                    siliconCords[i*dcA**2+j*dcA+k] = [i/2*siliconCellWidth,j/2*siliconCellWidth,k/2*siliconCellWidth]    
        tbd = np.full(len(siliconCords),0)
        tbdFullCount = 0
        for i in range(len(siliconCords)-1,0-1,-1):
            unevCordCount = 0
            for f in siliconCords[i]:
                if round((div:=f/siliconCellWidth),dec) != int(div):
                    unevCordCount += 1
            if unevCordCount == 1 or unevCordCount == 3:
                tbd[tbdFullCount] = i
                tbdFullCount += 1
        tbd = tbd[0:tbdFullCount+1]
        siliconCords2 = np.full((len(siliconCords)-len(tbd),3),0,dtype='float64')
        j = 0
        while i < len(siliconCords):
            if i not in tbd:
                siliconCords2[j] = siliconCords[i]
                j += 1
            i += 1
        siliconCords = siliconCords2
        cordsToAdd = np.full((1,3),-coloumbAmm*siliconCellWidth*100)
        maxC = max(siliconCords.flatten())
        minC = min(siliconCords.flatten())
        fCS = siliconCellWidth/4
        for e in siliconCords:
            unevCordCount = 0
            for f in e:
                if round((div:=f/siliconCellWidth),dec) != round(div):
                    unevCordCount += 1
            if unevCordCount == 0:
                cordsToAdd = np.append(cordsToAdd,[[e[0]+fCS,e[1]+fCS,e[2]+fCS]],axis=0)
                cordsToAdd = np.append(cordsToAdd,[[e[0]-fCS,e[1]-fCS,e[2]+fCS]],axis=0)
                cordsToAdd = np.append(cordsToAdd,[[e[0]+fCS,e[1]-fCS,e[2]-fCS]],axis=0)
                cordsToAdd = np.append(cordsToAdd,[[e[0]-fCS,e[1]+fCS,e[2]-fCS]],axis=0)
        for i in range(len(cordsToAdd)-1,0-1,-1):
            for e in cordsToAdd[i]:
                if e > maxC or e < minC:
                    cordsToAdd = np.delete(cordsToAdd,i,axis=0)
                    break
        siliconCords = np.append(siliconCords,cordsToAdd,axis=0)
        minusAmm = coloumbAmm//2*siliconCellWidth
        for i,__ in enumerate(siliconCords):
            siliconCords[i] -= minusAmm
        tempCords = np.absolute(siliconCords)
        tempCords2 = np.full(len(tempCords),0,dtype='float64')
        for i,e in enumerate(tempCords):
            tempCords2[i] = np.sum(e)
        sortIndicies = np.argsort(tempCords2)
        centralCorners = np.full((1,3),0,dtype='float64')
        for i in sortIndicies:
            evenCordsCount = 0
            for e in siliconCords[i]:
                if round((div:=e/siliconCellWidth),dec) == round(div):
                    evenCordsCount += 1
            if evenCordsCount == 3:
                centralCorners = np.append(centralCorners,[siliconCords[i]],axis=0)
            if len(centralCorners) >= 5:
                break 
        centralCorners = np.delete(centralCorners,0,axis=0)
        print(centralCorners)
        avgsCount = 3
        xPoint = 0
        yPoint = 0
        zPoint = 0
        for i in range(1,len(centralCorners)):
            if round(centralCorners[i][0],dec) != round(centralCorners[0][0],dec):
                xPoint = i
            if round(centralCorners[i][1],dec) != round(centralCorners[0][1],dec):
                yPoint = i
            if round(centralCorners[i][2],dec) != round(centralCorners[0][2],dec):
                zPoint = i
        counts = 0
        c = cKonst * siliconQ * -qe
        for x in frange(centralCorners[0][0],centralCorners[xPoint][0],(centralCorners[xPoint][0]-centralCorners[0][0])/avgsCount):
            for y in frange(centralCorners[0][1],centralCorners[yPoint][1],(centralCorners[yPoint][1]-centralCorners[0][1])/avgsCount):
                counts+=1
                for i in range(0,len(funcPots)):
                    z = i*stepSize + centralCorners[0][2]
                    for e in siliconCords:
                        divis = math.sqrt(math.pow(e[0]-x,2)+math.pow(e[1]-y,2)+math.pow(e[2]-z,2))
                        if divis != 0:
                            funcPots[i] += c/divis
                        else:
                            funcPots[i] = min(funcPots)
        for i,__ in enumerate(funcPots):
            funcPots[i] /= counts
        maxPot = max(funcPots)
        minPot = max(funcPots)
        for i,_ in enumerate(funcPots):
            if funcPots[i]-maxPot < coloumbCap:
                funcPots[i] = maxPot+coloumbCap
        for i,_ in enumerate(funcPots):
            funcPots[i] -= minPot

        """fig = plt.figure()
        ax = fig.add_subplot(111, projection='3d')
        Xs = siliconCords[0:len(siliconCords),0]
        Ys = siliconCords[0:len(siliconCords),1]
        Zs = siliconCords[0:len(siliconCords),2]
...
def tryAll():
    ERangeRad = 0.5
    maxE = -2.2*max(funcPots)
    minE = -3.5*max(funcPots)
    Estep = (maxE-minE)/100
