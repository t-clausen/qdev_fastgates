...
if "coloumbFunc.csv" in os.listdir(direct):
    ...
    coloumbPots = np.full(funcPots.shape,0,dtype='float64')
    ...
    for i,e in enumerate(coloumbPoints):
        ...
        for i,x in enumerate(frange(-limitDist, limitDist, stepSize)):
            if (r := abs(e+x)) != 0:
...
if case == "coloumb":
    ...
    for i in range(vertLines[0],vertLines[1]):
        funcPots[i] = min(coloumbPots)
    

plt.plot(funcCords,funcPots)
plt.plot(funcCords,coloumbPots)
plt.show() 
...
if __name__ == "__main__":
    ...
    while True:
        ...
        breakE = False
        if abs(func[len(func)-1] + func[0]) >= funcBorderFoundLimit:
            if len(EScores) >= 2:
